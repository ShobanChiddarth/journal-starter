...
async def analyze_entry(entry_id: str, entry_service: EntryService = Depends(get_entry_service)):
    ...
    temp_d = {}
    temp_d["work"] = result["work"]
    temp_d["struggle"]=result["struggle"]
    temp_d["intention"]=result["intention"]

    return await analyze_journal_entry(entry_id=entry_id, entry_text=json.dumps(temp_d))
